...
| 02 | `kafka-consumer-poison` | order with `X-User-Id: poison` → notification throws | 3 retries w/ backoff → DLT, partition unblocked; `POST /notifications/dlq/redrive` replays | head-of-line poison; DLQ + redrive policy | ◐ run 2026-07-15: retries + **partition-unblocked PROVEN** (SSE delivered right after poison). DLT publish itself still failing silently — recoverer throws even with topic pre-created; OPEN ITEM: debug DeadLetterPublishingRecoverer template wiring (suspect: partition-targeted publish vs topic metadata) |
